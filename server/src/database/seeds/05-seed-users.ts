...
export const USER_SEEDS: Array<
  Pick<User, 'email' | 'surname' | 'firstName' | 'isActive' | 'emailVerified'>
> = [
...
export async function seedUsers(dataSource: DataSource): Promise<void> {
  ...
  const users = USER_SEEDS.map((u) => repo.create({ ...u, passwordHash }));

  await repo.save(users);
  console.log(
    `  [users] Seeded ${users.length} users (password: ${DEFAULT_PASSWORD})`,
  );
}
