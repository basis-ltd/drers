...
export function buildUpdateDiff(
  ...
  const keys = new Set<string>([...Object.keys(oldFull), ...Object.keys(newFull)]);

  for (const key of keys) {
    if (IGNORED_DIFF_KEYS.has(key)) continue;
    const oldVal = (oldFull as Record<string, unknown>)[key];
    const newVal = (newFull as Record<string, unknown>)[key];
